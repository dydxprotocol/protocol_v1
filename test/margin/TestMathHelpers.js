...
contract('InterestHelper', (_accounts) => {
  ...
  describe('#divisionRoundedUp', () => {
    async function expectDivisionRoundedUp(num, den, expected) {
      const result = await contract.divisionRoundedUp.call(num, den);
  ...
  describe('#getNumBits', () => {
    async function expectBits(num, bits) {
      const result = await contract.getNumBits.call(num);
    ...
    it('works for 0-256 bits', async () => {
      const result = await contract.getNumBits.call(BIGNUMBERS.ZERO);
      expect(result).to.be.bignumber.equal(0);

      let n = new BigNumber(1);
      for (let i = 0; i < 256; i++) {
        const result = await contract.getNumBits.call(n);
