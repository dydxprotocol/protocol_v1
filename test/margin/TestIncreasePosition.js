...
async function setup(accounts, { loanOwner, positionOwner, depositInHeldToken } = {}) {
  ...
  increasePosTx.loanOffering.rates.minHeldToken = increasePosTx.loanOffering.rates.minHeldToken.div(2).floor();
  increasePosTx.loanOffering.signature = await signLoanOffering(increasePosTx.loanOffering);
...
async function validate({
  ...
  const finalHeldTokenPerUnit = getPartialAmount(finalBalance, (openTx.principal.plus(increasePosTx.principal)));
